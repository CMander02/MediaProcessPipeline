...
def test_desktop_build_hands_off_executable_and_matching_runtime():
    ...
    assert "Copy-Item -LiteralPath $RuntimeRoot -Destination $TemporaryRuntime -Recurse" in script
    assert "-RuntimePath $TemporaryRuntime" in script
    assert "Root runtime staging changed the attested runtime manifest" in script
    assert "Move-Item -LiteralPath $TemporaryRuntime -Destination $DestinationRuntime" in script
    assert "Move-Item -LiteralPath $PreviousRuntime -Destination $DestinationRuntime" in script
...
def test_root_launcher_requires_complete_desktop_artifact_pair():
    ...
    assert 'if exist "%APP_EXE%" if exist "%APP_RUNTIME_MANIFEST%" (' in launcher
