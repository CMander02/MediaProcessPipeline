...
`services/analysis/llm.py` 保留模型调用和分析阶段编排。字幕解析、说话人合并及润色约束由 transcript_outputs 处理，导图层级和章节组织由 mindmap_outputs 处理，分析采样与摘要 JSON 解析由 response_parsing 处理；LLMService 的方法签名继续兼容。
...
业务回归测试位于 tests/，前端行为测试随组件存放。开发分析与实施证据位于 agentspace/。任务历史以 SQLite 为准；旧 history.json 文件继续作为历史资料保留，归档扫描和目录布局识别保持兼容。
