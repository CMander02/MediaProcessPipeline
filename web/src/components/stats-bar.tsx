...
export function StatsBar({ stats }: { stats: TaskStats }) {
  return (
    <div className="flex items-center gap-4 text-sm tabular-nums">
      ...
      <Stat label="完成" value={stats.completed ?? 0} />
      <Stat label="处理中" value={stats.processing ?? 0} />
      <Stat label="排队" value={stats.queued ?? 0} />
      <Stat label="失败" value={stats.failed ?? 0} color="text-destructive" />
    </div>
  )
}
